...
describe("lifecycle completeness", () => {
  ...
  test("abort signal stops handler via onBeforeHandle guard", async () => {
    ...
    const dispose = peer.onDispose?.mock?.calls?.[0]?.[0];
    // Actually, let's just call the returned dispose() from bind()
    // but the return from bind() is the dispose fn. We didn't capture it.
    // Instead, we'll just test that onBeforeHandle can read signal.aborted.
