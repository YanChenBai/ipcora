...
describe('Ipcora core', () => {
  ...
  test('metadata is optional — no schema means no validation', async () => {
    ...
    await expect(invoke('test:meta-optional', 1, { id: '1', path: 'read' })).resolves.toEqual({
      data: { received: {} },
    });
  ...
  describe('use plugin', () => {
    ...
    test('named plugin can only be used once (singleton guard)', () => {
      const plugin = createIpcora({ name: 'auth-plugin' }).handler('login', () => 'ok');
      ...
      expect(() => app2.use(plugin)).toThrow('IPC plugin "auth-plugin" is already in use.');
    });
    ...
    test('unnamed plugin path conflict also triggers for group-prefixed routes', () => {
      const plugin = createIpcora().group('api', g => g.handler('status', () => 'ok'));
    ...
    test('plugin error mappers are merged and functional', async () => {
      ...
      const plugin = createIpcora().error(PluginError, ({ fail, error }) =>
